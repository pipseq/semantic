 ...
 * @see TestEvent
 */
public class ReleaseListener implements ModelChangedListener {
	private static final Logger log = LoggerFactory
			.getLogger(ReleaseListener.class);
	private int limit = 30;
	Pattern pattern = Pattern.compile(".*_[0-9][0-9][0-9]+$");	// min of three digits

	List<Statement> listStmt = new ArrayList<Statement>();

	public ReleaseListener(String regex, int limit) {
	...
	public void addedStatement(Statement s) {
		...
		if (m.matches()) {
			listStmt.add(s);
			if (listStmt.size() > limit) {
				Statement first = listStmt.remove(0);
				Model model = first.getModel();
				model.remove(first);
				log.debug("removing stmt for "
						+ getName(first));
			}
		}
	}
