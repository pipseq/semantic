 ...
 * @see TestEvent
 */
public class ReleaseBlankAndPredicateListener implements ModelChangedListener {
	private static final Logger log = LoggerFactory
			.getLogger(ReleaseBlankAndPredicateListener.class);
	private int limit = 30;
	Pattern pattern = Pattern.compile("TimeStamp");	// min of three digits

	List<Resource> listSubject = new ArrayList<Resource>();

	public ReleaseBlankAndPredicateListener(String regex, int limit) {
	...
	public void addedStatement(Statement s) {
		...
		if (sub.isAnon() && m.matches()) {
			listSubject.add(sub);
			if (listSubject.size() > limit) {
				Resource first = listSubject.remove(0);
				Model model = first.getModel();
				model.removeAll(first,null,null);
